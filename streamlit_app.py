...
st.set_page_config(page_title="魔釣 - 岡山・下津井タイラバ予報 v1.1", page_icon="🍑")
...
# 備讃瀬戸海上交通センター
KAIHO_URL = "https://www6.kaiho.mlit.go.jp/bisan/kisyou/choryu/choryu.html"
SEAT_CHECKER_URL = "" 
...
def get_seasonal_bait(month):
    ...
    if month in [12, 1, 2, 3]:
        ...
        return "真鯛の乗っ込み", "オレンジ・赤・ピンク"
    elif month in [6, 7, 8]:
        return "イワシ・イカ", "ゴールド・チャート・グロー"
    elif month in [9, 10, 11]:
        return "広範囲ベイト", "赤・オレンジ・エビオレ"
    else:
        return "混合", "赤オレ"
...
def suggest_strategy(h, sun_h, sc, t_diff, month, temp, cloud_cover, rain):
    """
    岡山・下津井特化ロジック
    - 特定メーカー名は出さず「定番」「シマシマ」と表記
    """
    c1 = "定番ピンク" if h % 2 == 0 else "オレンジ"
    s1 = "カーリー"
    speed = "普通"
    hook = "M"
    ...
    if rain >= 0.5:
        ...
        c1 = "チャート" if h % 2 == 0 else "定番ピンク"
    else:
        # 通常時はあのメーカー系の強い色
        c1 = "定番ピンク" if h % 2 == 0 else "オレンジゼブラ"
    ...
    # --- フックサイズ ---
    if month in [12, 1, 2, 3] and temp < 10.0:
        hook = "SS/3S"
    elif s1 == "極太ビッグ":
        hook = "L"
    elif sc >= 50:
    ...
    # カラーローテ
    if "ピンク" in c1: c2 = "オレンジ"
    elif "オレンジ" in c1: c2 = "レッド"
    elif "チャート" in c1: c2 = "ゴールド"
    elif "黒" in c1: c2 = "コーラ"
    else: c2 = "定番ピンク" # 迷ったらピンクに戻る
    ...
    if st.button("魔釣予報を開始する"):
        try:
            with st.spinner('瀬戸大橋周辺の海況を解析中...'):
                ...
                st.info("💡 **Tips:** 岡山・下津井エリアでは「定番メーカー」のピンク系（シマシマ含む）が圧倒的に強いです。必ず持参してください！")
